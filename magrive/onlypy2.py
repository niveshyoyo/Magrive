...
def process_link(magnet_link, email_id):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT FILE_NAME, DRIVE_LINK FROM magrive_1 WHERE TORRENT_LINK LIKE "' + magnet_link + '";');
    a = cursor.fetchall()

    if (len(a)!=0):
        send_mail(a[0][1], email_id, a[0][0] )
        share_link = a[0][1]
    else:
        ...
        os.system('rm -r data/*')
        cursor.execute('INSERT INTO magrive_1 (TORRENT_LINK, FILE_NAME, DRIVE_LINK) VALUES ("' + magnet_link + '","' + file[0]+ '","' + share_link +'")')
